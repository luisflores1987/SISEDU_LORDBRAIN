...
public class registroMatriculaServletController extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        if (action.equals("obtenerGradoPorNivel")) {
            obtenerGradoPorNivel(request, response);
        }
        ...
        if (action.equals("cargarAnioAcademico")) {
            cargarAnioAcademico(request, response);
        }
...
//        }               
    ...
    private void obtenerNivel(HttpServletRequest request, HttpServletResponse response) throws IOException {
        String sAñoAcademico = request.getParameter("anioAcademico") == null
                ? ""
                : request.getParameter("anioAcademico");

        List<Nivel> listaNivel = registroPension.obtenerNivelMatricula(sAñoAcademico);
        StringBuilder sb = new StringBuilder("");
        for (Nivel ln : listaNivel) {
            sb.append(
                    ln.getnIdNivel() + "-" + ln.getsDescripcion() + "-"
                    + ln.getsPension() + "-" + ln.getsAnioAcademico() + ":"
            );
        }
    ...
    private void obtenerGradoPorNivel(HttpServletRequest request, HttpServletResponse response) throws IOException {
...
//    }
    ...
    private void cargarAnioAcademico(HttpServletRequest request,
            HttpServletResponse response) throws IOException {
        List<String> lstMensaje = new ArrayList<String>();
        lstMensaje = registroPension.obtenerAnioAcademico();
        StringBuilder sb = new StringBuilder("");
        for (String sMsj : lstMensaje) {
            sb.append(sMsj + ";");
        }
        response.setContentType("text/plain");
        PrintWriter out = response.getWriter();
