...
public class obtenerNuevosRegistros extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        if (action.equalsIgnoreCase("obtenerAsistenciaAlumno")) {
            obtenerAsistenciaAlumno(request, response);
        }
        if (action.equalsIgnoreCase("colocarAsistenciaAlumno")) {
            colocarAsistenciaAlumno(request, response);
        }        
        ...
        if (action.equalsIgnoreCase("obtenerDatosAlumnoAsistencia")) {
            obtenerDatosAlumnoAsistencia(request, response);
        }        

        if (action.equalsIgnoreCase("colocarAsistenciaManual")) {
            colocarAsistenciaManual(request, response);
        }
        if (action.equalsIgnoreCase("colocarAsistenciaManualAlumno")) {
            colocarAsistenciaManualAlumno(request, response);
        }                
    }
    ...
    private void obtenerDatosAlumnoAsistencia(HttpServletRequest request, HttpServletResponse response) throws IOException {
        String sCodigo = request.getParameter("sCodigo");

        List<ProfesorHorario> listaProfesor = obtenerRegistro.obtenerAlumnoAsistencia(sCodigo);
        StringBuilder sb = new StringBuilder("");
        for (ProfesorHorario Proh : listaProfesor) {
            sb.append(
                    Proh.getProfesor().getPersona().getnIdPersona() + "+" //1
    ...
    private void colocarAsistenciaManualAlumno(HttpServletRequest request, HttpServletResponse response) throws IOException {
        String sCodigo = request.getParameter("sCodigo");
        String[] sMensaje = new String[2];
        sMensaje = obtenerRegistro.colocarAsistenciaManualAlumno(sCodigo);
        StringBuilder sb = new StringBuilder("");
        for (String sm : sMensaje) {
            sb.append(sm + "+");
        }
    ...
    private void obtenerAsistenciaAlumno(HttpServletRequest request, HttpServletResponse response) throws IOException {

        String codigoAsistencia = obtenerRegistro.obtenerRegistroAlumno();
        response.setContentType("text/plain");
        PrintWriter out = response.getWriter();
        out.print(codigoAsistencia);
    }

    private void colocarAsistenciaAlumno(HttpServletRequest request, HttpServletResponse response) throws IOException {
        String sCodigo = request.getParameter("sCodigo");
        String codigoAsistencia = obtenerRegistro.colocarAsistenciaAlumno(sCodigo);
        response.setContentType("text/plain");
        PrintWriter out = response.getWriter();
        out.print(codigoAsistencia);
    }    
    ...
    private void colocarEmpresa(HttpServletRequest request, HttpServletResponse response) throws IOException {
        String sMensaje = obtenerRegistro.colocarEmpresa();
        StringBuilder sb = new StringBuilder("");
        sb.append(sMensaje);
        response.setContentType("text/plain");
        PrintWriter out = response.getWriter();
