        ...
        if (action != null) {
            ...
            try {
                ...
                if (nIdNivel == 1) {
                    ...
                    reporFile = new File(rutaReportes + sTab + "reporteNotasLordBrain\\reporteNotasGradoInicial.jasper");
...
//                    reporFile = new File(request.getSession().getServletContext().getRealPath("/ReportesNotas/reporteNotasInicial.jasper"));
    ...
    private void reporteGrado(HttpServletRequest request, HttpServletResponse response) throws IOException {
        ...
        String sGrado = request.getParameter("sGrado").trim();    
        String sBimestre = request.getParameter("sBimestre").trim();            
        File reporFile = null;
        response.setHeader("Content-Disposition", "attachment; filename=\"ReporteDeNotas_" + sBimestre + "_" + sGrado  + ".pdf\";");
        ServletOutputStream out = response.getOutputStream();
        ...
        try {
            ...
            if (sIdNivel.equalsIgnoreCase("1")) {
                ...
                reporFile = new File(request.getSession().getServletContext().getRealPath("/ReportesNotas/reporteNotasWallon/reporteNotasGradoInicia_wallonl.jasper"));
            }
