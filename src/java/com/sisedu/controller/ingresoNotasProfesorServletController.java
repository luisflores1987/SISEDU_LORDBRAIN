...
public class ingresoNotasProfesorServletController extends HttpServlet {
    ...
    private void obtenerAlumnoxTutor(HttpServletRequest request, HttpServletResponse response) throws IOException {
        ...
        int nIdBimestre = Integer.valueOf(request.getParameter("IdBimestre"));
        System.out.println("request.getParameter(IdGrado) :  " + request.getParameter("IdGrado"));
        int nIdGrado = request.getParameter("IdGrado")==null?0:Integer.valueOf(request.getParameter("IdGrado"));        

        List<Alumno_Curso> listaAlumnoCurso = ingresoNotas.obtenerAlumnoxTutor(nIdPersona, nIdBimestre, nIdGrado);
        ...
        for (Alumno_Curso ac : listaAlumnoCurso) {
            sb.append(
                    ...
                    + ac.getsNota1() + "*"
                    + ac.getsNota2() + "*"
                    + ac.getAlumno().getSeccion().trim() + "*"
                    + ac.getsSeccionGrado().trim() + "*"
                    + ac.getsTardanzas() + "*"
