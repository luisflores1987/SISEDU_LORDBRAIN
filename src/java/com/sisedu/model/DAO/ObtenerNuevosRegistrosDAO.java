...
public class ObtenerNuevosRegistrosDAO {
   ...
   public List<ProfesorHorario> obtenerAlumnoAsistencia(String sCodigo) {
        Connection con = Conexion.conectar();
        listaProfesorHorario = new ArrayList<ProfesorHorario>();
        try {
            CallableStatement cs = con.prepareCall("{call [dbo].[usp_verificarAsistenciaAlumno](?, ?)}");
            cs.setInt(1, 3);
            cs.setString(2, sCodigo);
            ResultSet rs = cs.executeQuery();
            while (rs.next()) {
    ...
    public String[] colocarAsistenciaManualAlumno(String sCodigo) {
        Connection con = Conexion.conectar();
        String[] sResultado = new String[2];
        try {
            CallableStatement cs = con.prepareCall("{call [dbo].[usp_verificarAsistenciaAlumno](?, ?)}");
    ...
    public String obtenerRegistroAlumno() {
        Connection con = Conexion.conectar();
        String codigoProfesor = "";
        try {
            CallableStatement cs = con.prepareCall("{call [dbo].[usp_verificarAsistenciaAlumno](?)}");
    ...
    public String colocarAsistenciaAlumno(String sCodigo) {
        Connection con = Conexion.conectar();
        String sResultado = "";
        try {
            CallableStatement cs = con.prepareCall("{call [dbo].[usp_verificarAsistenciaAlumno](?, ?)}");
