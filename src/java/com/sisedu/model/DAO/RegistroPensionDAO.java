...
public class RegistroPensionDAO {
    ...
    public String ingresarPensionesMatricula(int nIdNivel,
            int nIdGrado,
            int nIdPersona,
            String sPension,
            int nIdAlumnoDeuda,
        ...
        try {
            ...
            CallableStatement cs
                    = conn.prepareCall("{call [dbo].[usp_filtroRegistroMatricula](?, ?, ?, ?, ?, ?, ?)}");
            if (nIdNivel != 0) { // ingreso por nivel
                cs.setInt(1, 1);
            } else if (nIdNivel == 0 && nIdGrado != 0) { // ingreso por grado
                cs.setInt(1, 2);
            } else if (nIdNivel == 0 && nIdGrado == 0 & nIdPersona != 0) { // ingreso por alumno
                cs.setInt(1, 3);
            } else {
                cs.setInt(1, 4);
            }
    ...
    public List<String> obtenerAnioAcademico() {

        List<String> lstMensaje = new ArrayList<String>();
        Connection conn = Conexion.conectar();
        try {
            CallableStatement cs = conn.prepareCall("{call [dbo].[usp_registroMatricula](?)}");
            cs.setInt(1, 11);
            ResultSet rs = cs.executeQuery();

            while (rs != null && rs.next()) {
                lstMensaje.add(rs.getString(1) + "," + rs.getString(2));
            }
            return lstMensaje;
        } catch (Exception ex) {
            throw new RuntimeException(ex);
